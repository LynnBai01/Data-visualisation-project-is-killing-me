...
ui <- dashboardPage(skin='green',
  ...
  dashboardSidebar(
    sidebarMenu(#menuItem('Charts',icon=icon('fas fa-chart-bar')),
                ...
                menuItem('Souce code for app', href='', icon=icon('github')),
                menuItem('Instruction video', href='', icon=icon('fab fa-youtube'))
  )),
  dashboardBody(
    verticalLayout(
...
server <- function(input, output) {
  ...
  output$Plot3 <- renderPlot({
    ...
    ggplot(DATA1,aes(x,y,col = Type, size = size)) + geom_point() + ylim(0,2300) +  xlim(0,2300) + xlab('') + ylab('') + 
      geom_text(aes(label = paste('x:',round(x,2),'y:',round(y,2),sep=' ')), nudge_y = 60,size = 3) 
  })
